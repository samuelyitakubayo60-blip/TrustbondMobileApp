...
const AuditLog = ({ wsRefreshKey }) => {
  ...
  const loadLogs = useCallback(() => {
    ...
    api
      .get(`/api/v1/audit-logs/?${params.toString()}`)
      .then((res) => {
        if (mounted) {
          setLogs(res || []);
  ...
  return (
    <>
      <div className="page-header">
        <h2>Audit Log</h2>
        <p>
          Tamper-evident record of all system actions for accountability and
          compliance.
        </p>
      </div>
      ...
      <div className="card">
          ...
          <button
            ...
            onClick={() => {
              ...
              const filtered = logs.filter((a) => {
                ...
                if (ent && (a.entity_type || "").toLowerCase() !== ent)
                  return false;
                if (act && (a.action_type || "").toLowerCase() !== act)
                  return false;
                if (!q) return true;
                const blob = [
                  a.actor_type,
            ...
            <tbody>
              {paginatedLogs.map((a, index) => (
                <tr key={a.log_id}>
                  ...
                  <td>
                    <span className="badge b-green" style={{ fontSize: "9px" }}>
                      {a.action_type}
                    </span>
                  </td>
                  ...
                  <td>
                    <span
                      className={`badge ${a.success ? "b-green" : "b-red"}`}
                      style={{ fontSize: "9px" }}
                    >
                      {a.success ? "Success" : "Failed"}
                    </span>
        ...
        >
          <div style={{ fontSize: "12px", color: "var(--muted)" }}>
            Showing {Math.min(offset + 1, filteredLogs.length)}-
            {Math.min(offset + pageSize, filteredLogs.length)} of{" "}
          ...
          <div className="pagination">
            ...
            {Array.from(
              {
                length: Math.min(5, Math.ceil(filteredLogs.length / pageSize)),
              },
              (_, i) => {
                const pageNum = i + 1;
            ...
            <button
              ...
              onClick={() =>
                setOffset(
                  Math.min(filteredLogs.length - pageSize, offset + pageSize),
                )
              }
