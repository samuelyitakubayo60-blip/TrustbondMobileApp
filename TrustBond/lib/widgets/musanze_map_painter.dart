...
/// Backward-compatible: assigns a stable (hash-based) color.
Color sectorColor(String sector, {double alpha = 1.0}) {
  final hash = sector.hashCode.abs();
  return MusanzeMapData.mapPaletteColor(hash, alpha: alpha);
}
...
class MusanzeMapPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final hotspot in sectorHotspots) {
      final latitude = JsonHelpers.doubleFromJson(hotspot, 'latitude');
      final longitude = JsonHelpers.doubleFromJson(hotspot, 'longitude');
      final riskLevel = hotspot['risk_level'] as String?;
      final incidentCount = JsonHelpers.intFromJson(hotspot, 'incident_count');
